...
BATCH = os.path.join(ROOT, ".scratch", "tag-card", "batches", "split-17.json")
OUT = os.path.join(ROOT, ".scratch", "tag-card", "judgments", "split-17.json")

NO_JA_NOTE = (
    "日文卡文缺漏(card_text_ja 為空):拆句表要求兩側原文的連續子字串與無遺漏覆蓋,"
    "日文側無從拆分對位;判定亦以日文卡文與補足情報為準(規範開頭),無從判。"
    "留待來源補齊日文後再拆(同票23-32 處理,票55 追蹤)。"
)
...
from _spec17 import fill  # noqa: E402
