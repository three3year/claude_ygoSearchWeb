...
from classify import (OLD_PATTERNS, TIER_NEW, TIER_OLD, TIER_REWRITTEN,
                      TIER_UNDATED, classify_card, is_effectless_monster,
                      old_pattern_labels)
...
def main(argv=None):
    ...
    (stats, seg_counts, card_sets, groups, dated, undated,
     signals) = scan(cards, dates)

    os.makedirs(os.path.dirname(args.out), exist_ok=True)
    with open(args.out, "w", encoding="utf-8", newline="\n") as f:
        f.write(render(stats, seg_counts, card_sets, groups, dated,
                       undated, signals))
