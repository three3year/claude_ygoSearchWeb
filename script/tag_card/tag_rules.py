...
# 召喚執行動詞形:「〜召喚する」須止於標點——「儀式召喚するモンスターの
# レベル」修飾形、「融合召喚する効果」描述形、「する場合」條件形都被邊界
# 擋;「融合召喚した」完成式=觸發天然不配(票20)。
_V_EXEC = r"召喚(する(?=[。、）」]|$)|できる(?=[。、）」]|$)|し、|し。)"
...
RULES = (
    ...
    define("T312", CAT_COUNTER, SCOPE_COST,
           ...
           {"act": CNT_RM}, "票19"),
    # ══ 第6期(票20):放置轉換 ══════════════════════════════════
    # 扱い[でと] 粒子錨:「永続罠カード扱いの場合」條件形(の格)天然不配;
    # 「装備されている」狀態形無動作動詞不配。
    define("T320", CAT_TRANSFORM, SCOPE_PROCESS,
           "裝備卡化「装備(魔法)カード扱いで/として…装備する」(「装備する"
           "効果を持つ」修飾形由負向斷言擋)",
           r"(装備カード|装備魔法カード)扱い[でと](して)?[^。]{0,60}?"
           r"装備(する(?!効果|事(は|が|も)?でき(ない|ず))|できる|し、|し。|し（)",
           {"to": TF_EQ}, "票20"),
    define("T321", CAT_TRANSFORM, SCOPE_PROCESS,
           "永續魔法化「永続魔法カード扱いで…置く」",
           r"永続魔法カード扱いで[^。]{0,50}?置(く|き)",
           {"to": TF_CS}, "票20"),
    define("T322", CAT_TRANSFORM, SCOPE_PROCESS,
           "永續陷阱化「永続罠カード扱いで…置く」",
           r"永続罠カード扱いで[^。]{0,50}?置(く|き)",
           {"to": TF_CTP}, "票20"),
    define("T323", CAT_TRANSFORM, SCOPE_PROCESS,
           "魔陷覆蓋「魔法カード扱いとして…セット」",
           r"魔法カード扱い[でと](して)?[^。]{0,40}?セット(する|できる|し、)",
           {"to": TF_SET}, "票20"),
    # ══ 第6期(票20):素材代用/召喚放寬 ══════════════════════════
    # 無專屬槽位,缺值只貼類別。完成形(リリースなしで召喚した)天然不配。
    define("T330", CAT_SUBSTITUTE, SCOPE_PROCESS,
           "無視召喚條件「召喚条件を無視して/し、…特殊召喚」(特召面照貼 mv)",
           r"召喚条件を無視(して|し、)[^。]{0,25}?特殊召喚(する|できる|し、|し。)",
           {}, "票20"),
    define("T331", CAT_SUBSTITUTE, SCOPE_PROCESS,
           "追加召喚權「通常召喚に加えて１度だけ…」",
           r"通常召喚に加えて",
           {}, "票20"),
    define("T332", CAT_SUBSTITUTE, SCOPE_PROCESS,
           "免解放召喚「リリースなしで召喚できる」",
           r"リリースなしで召喚できる",
           {}, "票20"),
    define("T333", CAT_SUBSTITUTE, SCOPE_PROCESS,
           "二體分解放「〜は２体分のリリースにできる」",
           r"２体分のリリースにでき(る|、)",
           {}, "票20"),
    define("T334", CAT_SUBSTITUTE, SCOPE_PROCESS,
           "素材來源放寬「(デッキ・墓地の〜も)融合素材とする事もできる」",
           r"素材とする事[もが]でき(る|、)",
           {}, "票20"),
    define("T335", CAT_SUBSTITUTE, SCOPE_PROCESS,
           "素材代用「融合素材モンスターの代わりにできる」(沼地型)",
           r"素材モンスター[^。]{0,10}?の代わりにでき(る|、)",
           {}, "票20"),
    # ══ 第6期(票20):召喚執行 ══════════════════════════════════
    # 照句形不問卡種(票20 裁定):魔陷與怪獸效果的執行動作同形同義。
    # method 照字面;「〜召喚扱いで特殊召喚」視同形照貼(RUM 重ねて形是
    # X 面主力),同句特召照貼 mv。
    define("T340", CAT_SUMMON_EXEC, SCOPE_PROCESS,
           "儀式召喚執行「儀式召喚する」",
           r"儀式" + _V_EXEC, {"method": SE_RIT}, "票20"),
    define("T341", CAT_SUMMON_EXEC, SCOPE_PROCESS,
           "融合召喚執行「融合召喚する」(「以外でも融合召喚できる」許可"
           "放寬形不是執行,排除)",
           r"融合" + _V_EXEC, {"method": SE_FUS}, "票20",
           exclude=r"以外でも[^。]{0,8}?融合召喚できる"),
    define("T342", CAT_SUMMON_EXEC, SCOPE_PROCESS,
           "同步召喚執行「S召喚する」",
           r"S" + _V_EXEC, {"method": SE_SYN}, "票20"),
    define("T343", CAT_SUMMON_EXEC, SCOPE_PROCESS,
           "超量召喚執行「(重ねて)X召喚する」",
           r"X" + _V_EXEC, {"method": SE_XYZ}, "票20"),
    define("T344", CAT_SUMMON_EXEC, SCOPE_PROCESS,
           "連結召喚執行「リンク召喚する」",
           r"リンク" + _V_EXEC, {"method": SE_LNK}, "票20"),
    define("T345", CAT_SUMMON_EXEC, SCOPE_PROCESS,
           "視同儀式召喚「儀式召喚扱いで特殊召喚」",
           r"儀式召喚扱い[でと](して)?[^。]{0,45}?特殊召喚",
           {"method": SE_RIT}, "票20"),
    define("T346", CAT_SUMMON_EXEC, SCOPE_PROCESS,
           "視同融合召喚「融合召喚扱いで特殊召喚」",
           r"融合召喚扱い[でと](して)?[^。]{0,45}?特殊召喚",
           {"method": SE_FUS}, "票20"),
    define("T347", CAT_SUMMON_EXEC, SCOPE_PROCESS,
           "視同同步召喚「S召喚扱いで特殊召喚」",
           r"S召喚扱い[でと](して)?[^。]{0,45}?特殊召喚",
           {"method": SE_SYN}, "票20"),
    define("T348", CAT_SUMMON_EXEC, SCOPE_PROCESS,
           "視同超量召喚「(重ねて)X召喚扱いで特殊召喚」(RUM 框架)",
           r"X召喚扱い[でと](して)?[^。]{0,45}?特殊召喚",
           {"method": SE_XYZ}, "票20"),
    define("T349", CAT_SUMMON_EXEC, SCOPE_PROCESS,
           "視同連結召喚「リンク召喚扱いで特殊召喚」",
           r"リンク召喚扱い[でと](して)?[^。]{0,45}?特殊召喚",
           {"method": SE_LNK}, "票20"),
    # ══ 第6期(票20):戰鬥規則 ══════════════════════════════════
    # 直接攻擊的許可與禁止都是戰鬥規則(批2);「直接攻撃宣言時」「攻撃対象
    # に選択された場合」觸發形與「(〜直接攻撃になる)」括弧註記天然不配。
    define("T350", CAT_BATTLE, SCOPE_PROCESS,
           "貫通「超えた分だけ戦闘ダメージを与える」(票17 歸戰鬥規則)",
           r"超えた分だけ[^。]{0,10}?戦闘ダメージを与え(る(?=[。、）]|$)|、)",
           {"what": BT_PI}, "票20"),
    define("T351", CAT_BATTLE, SCOPE_PROCESS,
           "連續攻擊「２/３回(まで)攻撃できる」",
           r"(２|３)回(まで)?攻撃でき(る(?=[。、）]|$)|、)",
           {"what": BT_MA}, "票20"),
    define("T352", CAT_BATTLE, SCOPE_PROCESS,
           "連續攻擊「もう１度(だけ続けて)攻撃できる」",
           r"もう１度[^。]{0,20}?攻撃でき(る(?=[。、）]|$)|、)",
           {"what": BT_MA}, "票20"),
    define("T353", CAT_BATTLE, SCOPE_PROCESS,
           "連續攻擊「(相手モンスター全てに)１回ずつ攻撃できる」",
           r"１回ずつ攻撃でき(る(?=[。、）]|$)|、)",
           {"what": BT_MA}, "票20"),
    define("T354", CAT_BATTLE, SCOPE_PROCESS,
           "直接攻擊許可「直接攻撃できる/でき、」",
           r"直接攻撃でき(る(?=[。、）]|$)|、)",
           {"what": BT_DA}, "票20"),
    define("T355", CAT_BATTLE, SCOPE_PROCESS,
           "直接攻擊禁止「直接攻撃できない」(許可與禁止都是戰鬥規則,批2)",
           r"直接攻撃でき(ない(?=[。、）]|$)|ず(?=[、。]))",
           {"what": BT_DA}, "票20"),
    define("T356", CAT_BATTLE, SCOPE_PROCESS,
           "攻擊對象遮蔽「攻撃対象に(選択)できない/ならない/されない」"
           "(批2:改攻擊對象選擇規則,非耐性)",
           r"攻撃対象に(な(ら|れ)(ない|ず)|されない|選択(でき|され)(ない|ず)"
           r"|でき(ない|ず)|指定でき(ない|ず))",
           {"what": BT_TA}, "票20"),
    define("T357", CAT_BATTLE, SCOPE_PROCESS,
           "攻擊對象移替「攻撃対象を…に移し替える」",
           r"攻撃対象を[^。]{0,25}?移し替え",
           {"what": BT_TA}, "票20"),
    define("T358", CAT_BATTLE, SCOPE_PROCESS,
           "攻擊對象強制/選擇權「攻撃対象にしなければならない/は自分が選択」",
           r"攻撃対象に[^。]{0,15}?しなければならない"
           r"|攻撃対象は[^。]{0,10}?選択す",
           {"what": BT_TA}, "票20"),
    # ══ 第6期(票20):情報操作 ══════════════════════════════════
    # what 照動詞字面(票20):確認する=確認;見せる/公開/めくる=展示;
    # ランダムに+動作=隨機決定。「見せた/めくった」完成形天然不配。
    define("T360", CAT_INFO, SCOPE_PROCESS,
           "確認「〜を確認する/して」",
           r"確認(する(?=[。、）」]|$)|し[、て]|できる(?=[。、）]|$))",
           {"what": IN_PK}, "票20"),
    define("T361", CAT_INFO, SCOPE_PROCESS,
           "展示「相手に見せる」(處理段;「見せる事ができる」選擇性動作"
           "照貼)",
           r"見せ(る(?=[。、）」]|$)|、|て|る事ができ(る|、))",
           {"what": IN_RV}, "票20"),
    define("T362", CAT_INFO, SCOPE_COST,
           "展示代價「相手に見せて発動/公開し続ける事で発動」",
           r"相手に見せ(て|、)|公開し続け",
           {"what": IN_RV}, "票20"),
    define("T363", CAT_INFO, SCOPE_PROCESS,
           "展示「公開する/し続ける」",
           r"公開(する(?=[。、）」]|$)|し、|し続け)",
           {"what": IN_RV}, "票20"),
    define("T364", CAT_INFO, SCOPE_PROCESS,
           "展示「めくる」",
           r"めく(る(?=[。、）」]|$)|り、|って)",
           {"what": IN_RV}, "票20"),
    define("T365", CAT_INFO, SCOPE_PROCESS,
           "隨機決定「ランダムに選ぶ/捨てる等」(副詞錨,動作動詞隨句)",
           r"ランダムに",
           {"what": IN_RD}, "票20"),
    # ══ 第6期(票20):其他 ══════════════════════════════════════
    # 無槽位只貼類別+pos。傷害修飾/轉嫁(票17)、存在限制(票18)、支付
    # 免除(票17)照舊裁歸此類;「コイントスを行う効果を持つ」修飾形與
    # 「行う度に」觸發形由動詞邊界擋。
    define("T370", CAT_MISC, SCOPE_PROCESS,
           "傷害修飾「(戦闘/効果/全ての)ダメージは/を０・半分・倍になる"
           "/にする」",
           r"ダメージ[はを][^。]{0,12}?(０|半分|倍)に"
           r"(な(る(?=[。、）]|$)|り、)|する(?=[。、）]|$)|し、"
           r"|でき(る)?(?=[。、）]|$))",
           {}, "票20"),
    define("T371", CAT_MISC, SCOPE_PROCESS,
           "戰鬥傷害轉嫁「戦闘ダメージは代わりに〜が受ける/も受ける」",
           r"戦闘ダメージは[^。]{0,12}?(代わりに[^。]{0,12}?受け(る|、)"
           r"|も受け(る|、)|お互いのプレイヤーが受け(る|、))",
           {}, "票20"),
    define("T372", CAT_MISC, SCOPE_PROCESS,
           "存在限制「〜しか(表側表示で)存在できない」(票18 歸其他)",
           r"しか[^。]{0,12}?存在できない",
           {}, "票20"),
    define("T373", CAT_MISC, SCOPE_PROCESS,
           "跳過階段/回合「フェイズ/ターンをスキップする」",
           r"(フェイズ|ターン)(１|２)?[をはも]?[^。]{0,4}?スキップ"
           r"(する(?=[。、）]|$)|し、|され)",
           {}, "票20"),
    define("T374", CAT_MISC, SCOPE_PROCESS,
           "擲硬幣「コイントスを行う」(擲的動作本身;結果效果各歸其類)",
           r"コイントスを[^。]{0,8}?行(う(?=[。、）]|$)|い、)",
           {}, "票20"),
    define("T375", CAT_MISC, SCOPE_PROCESS,
           "擲骰子「サイコロを振る」",
           r"サイコロを[^。]{0,10}?振(る(?=[。、）]|$)|り、|り直す)",
           {}, "票20"),
    define("T376", CAT_MISC, SCOPE_PROCESS,
           "支付免除「払うLPが必要なくなる」(票17 歸其他)",
           r"払うLPが必要なくな(る|り)",
           {}, "票20"),
)
...
_SCREENS = {
    ...
    # 第6期(票20):放置轉換——扱い[でと] 粒子錨;「永続罠カード扱いの場合」
    # 條件形(の格)天然不圈;裝備否定形不圈。
    CAT_TRANSFORM: (
        re.compile(r"(装備カード|装備魔法カード)扱い[でと]"
                   r"|永続魔法カード扱いで[^。]{0,50}?置"
                   r"|永続罠カード扱いで[^。]{0,50}?置"
                   r"|魔法カード扱い[でと](して)?[^。]{0,40}?セット"),
        re.compile(r"扱い[でと][^。]{0,60}?装備(する事)?[はがも]?"
                   r"でき(ない|ず)"),
    ),
    # 第6期(票20):素材代用/召喚放寬——完成形(リリースなしで召喚した)
    # 天然不圈;素材否定形歸行動限制(T237)不圈。
    CAT_SUBSTITUTE: (
        re.compile(r"召喚条件を無視|通常召喚に加えて"
                   r"|リリースなしで召喚できる|２体分のリリースにでき"
                   r"|素材とする事[もが]でき"
                   r"|素材モンスター[^。]{0,10}?の代わりにでき"),
        re.compile(r"素材とする事[もが]でき(ない|ず)"),
    ),
    # 第6期(票20):召喚執行——動作形止於標點;修飾形(儀式召喚する
    # モンスター)與完成式(融合召喚した=觸發)天然不圈。
    CAT_SUMMON_EXEC: (
        re.compile(r"(儀式|融合|S|X|リンク)召喚(する(?=[。、）」]|$)"
                   r"|できる(?=[。、）」]|$)|し、|し。)"
                   r"|(儀式|融合|S|X|リンク)召喚扱い[でと]"),
        re.compile(r"召喚する効果を含む|以外でも[^。]{0,8}?融合召喚できる"),
    ),
    # 第6期(票20):戰鬥規則——「直接攻撃宣言時」「攻撃対象に選択された」
    # 觸發形天然不圈;「(〜直接攻撃になる)」括弧註記由排除樣式剝掉。
    CAT_BATTLE: (
        re.compile(r"超えた分だけ[^。]{0,10}?戦闘ダメージを与え"
                   r"|(２|３)回(まで)?攻撃でき|もう１度[^。]{0,20}?攻撃でき"
                   r"|１回ずつ攻撃でき|直接攻撃でき"
                   r"|攻撃対象に(な(ら|れ)(ない|ず)|されない"
                   r"|選択(でき|され)(ない|ず)|でき(ない|ず)"
                   r"|指定でき(ない|ず))"
                   r"|攻撃対象を[^。]{0,25}?移し替え"
                   r"|攻撃対象に[^。]{0,15}?しなければならない"
                   r"|攻撃対象は[^。]{0,10}?選択す"),
        re.compile(r"直接攻撃になる"),
    ),
    # 第6期(票20):情報操作——「見せた/めくった/確認した」完成形天然不圈;
    # 「宣言する」值域無對應值不建模不圈(記錄殘餘)。
    CAT_INFO: (
        re.compile(r"確認(する|し[、て]|できる)|見せ(る|、|て)"
                   r"|公開(する|し、|し続け)|めく(る|り、|って)|ランダムに"),
        re.compile(r"確認した(場合|時)"),
    ),
    # 第6期(票20):其他——傷害修飾/轉嫁、存在限制、跳過、硬幣/骰子、支付
    # 免除的具名句形;「行う効果/度に」修飾與觸發形不圈。縦列句形態異質
    # 不圈不貼(記錄殘餘,票20)。
    CAT_MISC: (
        re.compile(r"ダメージ[はを][^。]{0,12}?(０|半分|倍)に"
                   r"(な|する|し、|でき)"
                   r"|戦闘ダメージは[^。]{0,12}?(代わりに|も受け"
                   r"|お互いのプレイヤーが受け)"
                   r"|しか[^。]{0,12}?存在できない"
                   r"|(フェイズ|ターン)(１|２)?[をはも]?[^。]{0,4}?スキップ"
                   r"|コイントスを[^。]{0,8}?行(う(?=[。、）]|$)|い、|わず)"
                   r"|サイコロを[^。]{0,10}?振(る(?=[。、）]|$)|り、|り直)"
                   r"|払うLPが必要なくな"),
        re.compile(r"ダメージ[はを][^。]{0,12}?(０|半分|倍)にな(った|る場合)"),
    ),
}
